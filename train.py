...
from fusionet2 import FusionModel, nnunet_binary_loss
...
def train():
    ...
    num_classes = 7 # 0:bg, 1:noise, 2-6: 5 vessel classes
    ...
    for epoch in range(num_epochs):
        ...
        for i, (images, masks_dict, _) in enumerate(pbar):
            ...
            with torch.amp.autocast('cuda', enabled=(device.type == 'cuda')):
                output, ortho_loss, noise_out = model(images, return_noise_logit=True)
                ...
                # 7-class segmentation loss (shared heads; noise channel detached → no gradient to nnUNet)
                seg_loss = criterion(output, mask_full)
                
                # nnUNet dedicated binary loss (Dice + BCE) — only source of gradient for nnUNet
                noise_gt = masks_dict['noise'].to(device).float().unsqueeze(1)
                nnunet_loss = nnunet_binary_loss(noise_out, noise_gt)

                ortho_weight = 0.6
                nnunet_weight = 0.1
                
                loss = seg_loss + (ortho_weight * ortho_loss) + (nnunet_weight * nnunet_loss)
            ...
            pbar.set_postfix({
                ...
                "nnunet": f"{nnunet_loss.item():.4f}"
            })

            # Optimization: Clear unused tensors AFTER logging
            del output, ortho_loss, seg_loss, nnunet_loss, noise_out, loss

            # Optional: Periodic memory cleanup within epoch
            if i % 100 == 0:
